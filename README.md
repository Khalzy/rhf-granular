...
rhf-granular works by subscribing to specific slices of form state. Before writing any code, read the [footguns](#footguns) section. A few common patterns will silently cause full re-renders if you're not aware of them.
...
### The initialization double render
...
### Avoid reading formState at the top level
...
### Avoid using useFormContext for reactive reads
...
### Avoid destructuring fields from useFieldArray for granular reads
...
### Use getFieldState inside useFormSelector for field-level state
...
```tsx
// When you need a controlled input
const { field, fieldState } = useController({ name: "username", control });

// When you only need field state
